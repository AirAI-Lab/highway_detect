...
class MyFrame():
    def __init__(self, net, loss, lr=2e-4, evalmode=False, device=None):
        # choose device
        ...
        if self.device.type == 'cuda':
            model = model.cuda()
            # use DataParallel only when cuda is available
            if torch.cuda.device_count() > 1:
                model = torch.nn.DataParallel(model, device_ids=range(torch.cuda.device_count()))
        # keep CPU model as-is
        self.net = model
    ...
    def load(self, path):
        # load respecting current device
        state = torch.load(path, map_location=self.device)
        try:
            self.net.load_state_dict(state)
        except Exception:
            # try non-strict for minor mismatches
