...
class BiReNet34(nn.Module):
    def __init__(self, out_channels=1, pretrained: bool = False):
        ...
        resnet = load_resnet34_backbone(pretrained=bool(pretrained), caller='BiReNet.BiReNet34')
    ...
    def forward(self, x):
        ...
        # Decoder
        if self.has_FFM:
            d4 = self.FeatureFusionModule3(self.decoder4(e4), e3)
            d3 = self.FeatureFusionModule2(self.decoder3(d4), e2)
            d2 = self.FeatureFusionModule1(self.decoder2(d3), e1)
        else:
