...
from .swin_aspp.swin_aspp_3 import build_aspp
from .swin_aspp.swin_configs_3 import ASPPConfig
import numpy as np
...
class DAM(nn.Module):
    ...
    def forward(self, c_x, d_x):
        ...
        x1 = nonlinearity(self.bn1(x1))
        x1 = self.sigmoid1(x1)
        # 保证与 c_x 的空间尺寸一致（当输入尺寸非方形或与设定的 c_size 不一致时，需要插值对齐）
        if x1.shape[2:] != c_x.shape[2:]:
            x1 = F.interpolate(x1, size=c_x.shape[2:], mode='bilinear', align_corners=False)
        x1 = torch.mul(c_x, x1)
        x2 = self.GPA2(c_x)
        x2 = self.conv2(x2)
        x2 = nonlinearity(self.bn2(x2))
...
class DAM_Net_5(nn.Module):
    # 默认就是1，num_classes
    def __init__(self, img_size=1024, num_classes=BINARY_CLASS, pretrained: bool = False):
        super(DAM_Net_5, self).__init__()
        filters = [64, 128, 256, 512]
        # Prefer local ResNet34 weights only when explicitly requested; otherwise random-init
        resnet = load_resnet34_backbone(pretrained=bool(pretrained), caller='CARNet.DAM_Net_5')
