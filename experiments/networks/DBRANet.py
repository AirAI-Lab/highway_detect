...
class LinkNet34(nn.Module):
    def __init__(self, num_classes=1):
        ...
        # Use shared local loader; keep pretrained=False here by default
        resnet = load_resnet34_backbone(pretrained=True, caller='DBRANet.LinkNet34')
...
class LocalAttentionDecoder(nn.Module):
    ...
    def _enlarge_map(self, pooling_map, feature_map_size):
        ...
        a = pooling_map.view(B, C, 1, -1).repeat(1, 1, basic, 1).view(B, C, -1, 1).repeat(1, 1, 1, basic).view(B, C,
                                                                                                               feature_map_size,
                                                                                                               feature_map_size)
        # ensure index tensor is on the same device as pooling_map (CPU or CUDA)
        b = torch.tensor(self._generate_sequence(H, basic), device=pooling_map.device)
...
class DBRANet_dual(nn.Module):
    def __init__(self, num_classes=1):
        ...
        layers = [3, 4, 6, 3]
        # prefer local ImageNet weights if available; gracefully fallback
        top_residual_branch = load_resnet34_backbone(pretrained=True, caller='DBRANet_dual')
...
class DBRANet_dual_PW(nn.Module):
    def __init__(self, num_classes=1):
        ...
        filters = [64, 128, 256, 512]
        layers = [3, 4, 6, 3]
        top_residual_branch = load_resnet34_backbone(pretrained=True, caller='DBRANet_dual_PW')
        bottom_refinedasymmetric_branch = RefinedAsymmetricBlock()
...
class RACBranch(nn.Module):
    def __init__(self, num_classes=1):
        ...
        filters = [64, 128, 256, 512]
        layers = [3, 4, 6, 3]
        top_residual_branch = load_resnet34_backbone(pretrained=True, caller='RACBranch')
        bottom_refinedasymmetric_branch = RefinedAsymmetricBlock()
...
class DBRANet(nn.Module):
    def __init__(self, pool_size=4, pretrained: bool = False):
        super(DBRANet, self).__init__()

        filters = [64, 128, 256, 512, 1024]
        layers = [3, 4, 6, 3]
        top_residual_branch = load_resnet34_backbone(pretrained=bool(pretrained), caller='DBRANet')
...
class DBRANet_PW(nn.Module):
    def __init__(self, pool_size):
        ...
        filters = [64, 128, 256, 512]
        layers = [3, 4, 6, 3]
        top_residual_branch = load_resnet34_backbone(pretrained=True, caller='DBRANet_PW')
        bottom_refinedasymmetric_branch = RefinedAsymmetricBlock()
...
class DBRANet_RAD(nn.Module):
    def __init__(self, pool_size):
        ...
        filters = [64, 128, 256, 512]
        resnet = load_resnet34_backbone(pretrained=True, caller='DBRANet_RAD')
        self.firstconv = resnet.conv1
        self.firstbn = resnet.bn1
        self.firstrelu = resnet.relu
        self.firstmaxpool = resnet.maxpool
...
class DBRANet_PW_Fuse(nn.Module):
    def __init__(self, pool_size=4):
        ...
        filters = [64, 128, 256, 512]
        layers = [3, 4, 6, 3]
        top_residual_branch = load_resnet34_backbone(pretrained=True, caller='DBRANet_PW_Fuse')
        bottom_refinedasymmetric_branch = RefinedAsymmetricBlock()
