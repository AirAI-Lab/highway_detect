...
class DSConv(nn.Module):
    ...
    def forward(self, f):
        # Derive device from input feature to support CPU-only runs
        current_device = f.device
        offset = self.offset_conv(f)
        offset = self.bn(offset)
        # We need a range of deformation between -1 and 1 to mimic the snake's swing
        offset = torch.tanh(offset)
        input_shape = f.shape
        # Always construct DSC with the runtime device inferred from input feature
