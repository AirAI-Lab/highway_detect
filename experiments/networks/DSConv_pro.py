...
class DSConv(nn.Module):
    def __init__(
        ...
        # Device preference for coordinate map creation; actual tensors follow input's device.
        try:
            if isinstance(device, torch.device):
                self.device_pref = device.type
            elif isinstance(device, str):
                if device.lower() == "auto":
                    self.device_pref = "cuda" if torch.cuda.is_available() else "cpu"
                elif device.lower() == "cuda" and not torch.cuda.is_available():
                    self.device_pref = "cpu"
                else:
                    self.device_pref = device.lower()
            else:
                self.device_pref = "cpu"
        except Exception:
            self.device_pref = "cpu"
...
def get_coordinate_map_2D(
    ...
    dev = offset.device if device is None else torch.device(device)

    y_offset_, x_offset_ = torch.split(offset, kernel_size, dim=1)

    y_center_ = torch.arange(0, width, dtype=torch.float32, device=dev)
    y_center_ = einops.repeat(y_center_, "w -> k w h", k=kernel_size, h=height)

    x_center_ = torch.arange(0, height, dtype=torch.float32, device=dev)
    ...
    if morph == 0:
        ...
        """
        y_spread_ = torch.zeros([kernel_size], device=dev)
        x_spread_ = torch.linspace(-center, center, kernel_size, device=dev)
        ...
        Initialize the kernel and flatten the kernel
            ...
            x: only need 0
        """
        y_spread_ = torch.linspace(-center, center, kernel_size, device=dev)
        x_spread_ = torch.zeros([kernel_size], device=dev)
