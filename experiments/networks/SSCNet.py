...
from .DSConv_pro import DSConv
from functools import partial
import os
from collections import OrderedDict
# Prefer new timm.layers import path; fall back for older timm versions
try:
    from timm.layers import SqueezeExcite, CondConv2d
...
class SSCNet(nn.Module):   #StratoSyncCrossNet
    def __init__(self, num_classes=1, pretrained: bool = False):
        super(SSCNet, self).__init__()
        # 1024*3
        filters = [48, 64, 160, 1280]
        # Prefer local pretrained weights only when requested; otherwise random init
        try:
            model = models.efficientnet_v2_s(weights=None)
        except TypeError:  # torchvision < 0.13 API
            model = models.efficientnet_v2_s(pretrained=False)

        if bool(pretrained):
            # Try loading local weights file: experiments/weights/efficientnet_v2_s-dd5fe13b.pth
            file_dir = os.path.dirname(os.path.abspath(__file__))
            exp_dir = os.path.dirname(file_dir)
            weights_path = os.path.join(exp_dir, 'weights', 'efficientnet_v2_s-dd5fe13b.pth')
            if not os.path.isfile(weights_path):
                # record not_found for unified fallback status
                try:
                    from . import weights_helper as _wh
                    _wh._append_status({'caller': 'SSCNet', 'variant': 'efficientnet_v2_s', 'event': 'not_found', 'path': weights_path.replace('\\','/'), 'strict': False, 'exception': None})
                except Exception:
                    pass
            else:
                try:
                    ...
                    def _strip_prefix(d, prefix):
                        if any(k.startswith(prefix) for k in d.keys()):
                            return OrderedDict((k[len(prefix):], v) if k.startswith(prefix) else (k, v) for k, v in d.items())
                        return d
                    ...
                    try:
                        ...
                        # record status for unified pretrained reporting
                        try:
                            from . import weights_helper as _wh
                            _wh._append_status({'caller': 'SSCNet', 'variant': 'efficientnet_v2_s', 'event': 'loaded', 'path': weights_path.replace('\\','/'), 'strict': False, 'exception': None})
                        except Exception:
                            pass
                    except Exception as e:
                        # record load_failed and continue
                        try:
                            from . import weights_helper as _wh
                            _wh._append_status({'caller': 'SSCNet', 'variant': 'efficientnet_v2_s', 'event': 'load_failed', 'path': weights_path.replace('\\','/'), 'strict': False, 'exception': repr(e)})
                        except Exception:
                            pass
                except Exception as e:
                    # record load_failed on torch.load error
                    try:
                        from . import weights_helper as _wh
                        _wh._append_status({'caller': 'SSCNet', 'variant': 'efficientnet_v2_s', 'event': 'load_failed', 'path': weights_path.replace('\\','/'), 'strict': False, 'exception': repr(e)})
                    except Exception:
    ...
    def forward(self, x):
        ...
        #Center
        cmgi = self.cmgi(e4, att3, att4)

        # Decoder
        d3 = self.decoder4(att4+cmgi)# 32 32 1280 => # 64 64 160 #e4要改
...
class DRDM(nn.Module):  #Dynamic Road Detail Matcher
    def __init__(self, num: int, channels: int):
        ...
        self.dsconv13 = DSConv(channels, channels, kernel_size=15 - 4 * num, extend_scope=1,
                               morph=0, if_offset=True, device="auto")
        ...
        self.dsconv23 = DSConv(channels, channels, kernel_size=15 - 4 * num, extend_scope=1,
                               morph=1, if_offset=True, device="auto")
...
class CMGI(nn.Module): #Comprehensive Multi-Scale Global Integrator
    def __init__(self):
