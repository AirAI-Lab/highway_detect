...
try:
...
except Exception:  # lightweight fallback if mmcv is not available
    import torch.nn as nn
    ...
    class ConvModule(nn.Sequential):
        def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0,
            ...
            # basic BN + ReLU as a minimal replacement
        ...
        if bool(pretrained):
            ...
            if not os.path.isfile(weights_path):
                ...
                try:
                    ...
                    _wh._append_status({'caller': 'WeavingUnet', 'variant': 'efficientnet_v2_s', 'event': 'not_found', 'path': weights_path.replace('\\','/'), 'strict': False, 'exception': None})
                except Exception:
                    pass
            else:
                try:
                    ...
                    # strip potential prefixes that appear in many checkpoints
                    ...
                    try:
                        ...
                        try:
                            from . import weights_helper as _wh
                            _wh._append_status({'caller': 'WeavingUnet', 'variant': 'efficientnet_v2_s', 'event': 'loaded', 'path': weights_path.replace('\\','/'), 'strict': False, 'exception': None})
                        except Exception:
                            pass
                        ...
                        try:
                            from . import weights_helper as _wh
                            _wh._append_status({'caller': 'WeavingUnet', 'variant': 'efficientnet_v2_s', 'event': 'load_failed', 'path': weights_path.replace('\\','/'), 'strict': False, 'exception': repr(e)})
                        except Exception:
                            pass
                except Exception as e:
