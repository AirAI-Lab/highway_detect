...
class Swin_DAC_Block(nn.Module):
    def __init__(self, img_size=32, zero_head=False, patch_size=4,
                 patch_chans=512, norm_layer=nn.LayerNorm, patch_norm=True, drop_rate=0.,
                 drop_path_rate=0.1, depths=[2], ape=False, num_heads=[4], window_size=8,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, attn_drop_rate=0., use_checkpoint=False):
        super(Swin_DAC_Block, self).__init__()
        ...
        self.conv1 = nn.Conv2d(patch_chans * 2, patch_chans // 4, kernel_size=1)
        ...
        self.swin_DAC_block = nn.ModuleList()

        for i in range(self.num_layers):
            swin_block1 = BasicLayer(dim=int(embed_dim * 2 ** i),
            ...
            self.swin_DAC_block.append(swin_block1)

        for i in range(self.num_layers):
            swin_block2 = BasicLayer(dim=int(embed_dim * 2 ** i),
                                     input_resolution=(patches_resolution[0] // (2 ** i),
                                                       patches_resolution[1] // (2 ** i)),
                                     depth=depths[i],
                                     num_heads=num_heads[i],
                                     ...
                                     norm_layer=norm_layer,
                                     use_checkpoint=use_checkpoint)
            self.swin_DAC_block.append(swin_block2)

    def forward(self, x, dam_x):
        x = torch.cat((x, dam_x), 1)
        x = self.conv1(x)
        x = self.norm1(x)
        x = self.relu1(x)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=512, patch_size=4, in_chans=128, embed_dim=2048, norm_layer=None):
        super().__init__()
...
if __name__ == '__main__':
    ...
    model = Swin_DAC_Block()
    summary(model, input_size=[(1, 512, 32, 32),(1, 512, 32, 32)])
